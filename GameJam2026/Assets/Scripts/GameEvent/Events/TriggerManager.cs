...
public class TriggerManager : MonoSingleton<TriggerManager>
{
    ...
    {
        EventManager.Subscribe<TriggerEvent>(OnTriggerEvent);
    }

    public override void ClearSingleton()
    {
        EventManager.Unsubscribe<TriggerEvent>(OnTriggerEvent);
    }
    ...
    private void HandlePoint(TriggerEvent evt)
    {
        ...
        EventManager.Raise(
            new EntityConsumedEvent(
                evt.Trigger.gameObject,
                evt.TriggeredObject.gameObject
            )
        );
    }
